...
from categories
-- =============================================================================
-- GROUP CONTRIBUTIONS: PERSONAL WALLET -> GROUP WALLET + REVERSE SUPPORT
-- Apply this to your existing database, and merge the same changes into BudgetF.sql.
-- =============================================================================

-- 1) Extend group_contributions audit/source columns.
ALTER TABLE group_contributions
  ADD COLUMN IF NOT EXISTS source_wallet_id BIGINT;

ALTER TABLE group_contributions
  ADD COLUMN IF NOT EXISTS personal_transaction_id BIGINT;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'fk_group_contributions_source_wallet'
  ) THEN
    ALTER TABLE group_contributions
      ADD CONSTRAINT fk_group_contributions_source_wallet
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'fk_group_contributions_personal_transaction'
  ) THEN
    ALTER TABLE group_contributions
      ADD CONSTRAINT fk_group_contributions_personal_transaction
      FOREIGN KEY (personal_transaction_id)
      REFERENCES transactions(transaction_id)
      ON DELETE SET NULL;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'fk_group_contributions_reversed_by'
  ) THEN
    ALTER TABLE group_contributions
      ADD CONSTRAINT fk_group_contributions_reversed_by
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'ck_group_contributions_status'
  ) THEN
    ALTER TABLE group_contributions
      ADD CONSTRAINT ck_group_contributions_status
      CHECK (status IN ('completed', 'reversed'));
  END IF;
END;
$$;

CREATE INDEX IF NOT EXISTS ix_group_contributions_source_wallet
ON group_contributions(source_wallet_id);

CREATE INDEX IF NOT EXISTS ix_group_contributions_personal_tx
ON group_contributions(personal_transaction_id);

CREATE INDEX IF NOT EXISTS ix_group_contributions_status
ON group_contributions(status);

-- 2) Wallet balance trigger must count only completed contributions.
CREATE OR REPLACE FUNCTION group_contribution_effect(p_status TEXT, p_amount NUMERIC)
RETURNS NUMERIC AS $$
BEGIN
...
BEGIN
  IF TG_OP IN ('UPDATE', 'DELETE') THEN
    v_old_effect := group_contribution_effect(OLD.status, OLD.amount);
    PERFORM apply_group_wallet_delta(OLD.group_wallet_id, -v_old_effect);
  END IF;

  IF TG_OP IN ('INSERT', 'UPDATE') THEN
    v_new_effect := group_contribution_effect(NEW.status, NEW.amount);
    PERFORM apply_group_wallet_delta(NEW.group_wallet_id, v_new_effect);
  END IF;
...
-- 3) Assignment recompute must count only completed contributions.
CREATE OR REPLACE FUNCTION recompute_contribution_assignment(p_assignment_id BIGINT)
RETURNS VOID AS $$
DECLARE
  v_expected NUMERIC(14,2);
...
DROP TRIGGER IF EXISTS trg_group_contribution_wallet_balance ON group_contributions;
CREATE TRIGGER trg_group_contribution_wallet_balance
AFTER INSERT OR UPDATE OF amount, group_wallet_id, status OR DELETE
ON group_contributions
FOR EACH ROW EXECUTE FUNCTION trgfn_group_contribution_wallet_balance();

DROP TRIGGER IF EXISTS trg_group_contribution_assignment_recompute ON group_contributions;
CREATE TRIGGER trg_group_contribution_assignment_recompute
AFTER INSERT OR UPDATE OF amount, assignment_id, status OR DELETE
ON group_contributions
FOR EACH ROW EXECUTE FUNCTION trgfn_recompute_assignment_after_contribution();

-- Recompute current assignment values after migration, in case reversed rows already exist.
SELECT recompute_contribution_assignment(assignment_id)
FROM group_contribution_assignments;
